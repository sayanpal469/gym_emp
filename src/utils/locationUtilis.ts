...
class AndroidUniversalSupport {
  ...
  static async requestUniversalLocationPermission(): Promise<boolean> {
    ...
    try {
      ...
      console.log(
        `📱 Permission ${hasPermission ? 'granted' : 'denied'} for ${
    ...
    Geolocation.getCurrentPosition(
      ...
      error => {
        ...
        if (versionInfo.isPreMarshmallow) {
          ...
          message =
            'Swipe down and enable Location, or go to Settings > Location.';
        }

        Alert.alert('Location Services Required', message, [
    ...
    return new Promise((resolve, reject) => {
      ...
      Geolocation.getCurrentPosition(
        position => {
          ...
          console.log(
            `📍 Optimized - Accuracy: ${accuracy ? accuracy + 'm' : 'Unknown'}`,
          );
...
// Utility function - Calculate distance between two coordinates in meters
...
// Check if current location is within allowed distance of any branch
export const isWithinAllowedDistance = (
  currentLocation: Coordinates,
  branches: Branch[],
  maxDistanceMeters: number = 100,
): { isWithin: boolean; nearestBranch: Branch | null; distance: number } => {
  if (!branches || branches.length === 0) {
    return { isWithin: false, nearestBranch: null, distance: Infinity };
  }
  ...
  return {
    isWithin: minDistance <= maxDistanceMeters,
    nearestBranch,
...
export const getLocationManually = async (): Promise<Coordinates | null> => {
  try {
    ...
    const hasPermission =
      await AndroidUniversalSupport.requestUniversalLocationPermission();
    if (!hasPermission) {
      Alert.alert(
    ...
    if (location) {
      console.log(
        `✅ Successfully got location on ${versionInfo.versionName}:`,
        location,
      );
      return location;
    } else {
    ...
    Alert.alert('Location Unavailable', helpMessage, [
      {
        text: 'Try Again',
        style: 'default',
      },
